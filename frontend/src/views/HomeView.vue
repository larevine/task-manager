...
console.log(1, { rt: rawTasks }, { nt: normalizedTasks }, {f:normalizedTasks
        .filter(({ columnId }) => columnId)});

// filter removes from this array all elements whose `columnId` field is `false`, `null`, `0`, `undefined`, `NaN` or the empty string.
const columnTasks = normalizedTasks
  ...
  .reduce((accumulator, task) => {
    ...
    if (accumulator[task.columnId]) {
      accumulator[task.columnId] = [...accumulator[task.columnId], task]; // used to create a new array from an old one, and add a new `task` element.
    } else {
